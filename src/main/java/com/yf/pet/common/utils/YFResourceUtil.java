...
public class YFResourceUtil {
    ...
    private static final org.slf4j.Logger log = LoggerFactory.getLogger(YFResourceUtil.class);
     ...
     * @user wupengyu
     */
    public static void writerValueWithFullPath(String path, String key, String value) {
        try {
            File f = new File(path);
            if (f.exists() && f.canRead()) {
                writerValue(f, key, value);
            }
        } catch (Exception e) {
            log.error("", e);
        }
    ...
    public static Map<String, String> getPropertieByFullPath(String filePath) {
        Map<String, String> resultMap = new HashMap<String, String>();
        try {
            File f = new File(filePath);
            if (f.exists() && f.canRead()) {
                if (f.getName().trim().contains(".properties")) {
                    Properties properties = YFResourceUtil.getProperties(f);
                    for (Object key : properties.keySet()) {
                        resultMap.put(key.toString(), properties.get(key).toString());
                    }
                }
     ...
     * @user wupengyu
     */
    public static String getValueByKeyWithFullPath(String filePath, String key) {
        String result = null;
        try {
            File f = new File(filePath);
            if (f.exists() && f.canRead()) {
                result = getValueByKey(f, key);
            }
        } catch (Exception e) {
            log.error("读取资源文件[" + filePath + "]失败!!!", e);
        }
