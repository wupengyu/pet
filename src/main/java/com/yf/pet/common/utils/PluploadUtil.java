...
import com.yf.pet.common.entity.Plupload;
...
/**
 * Plupload是一个上传插件。 上传原理为单个文件依次发送至服务器. 上传打文件时可以将其碎片化上传。 但是一般情况下，不会这样做， 所以这里更多的是处理普通文件的批量上传。 这里主要处理文件上传
 * 
 * @author infi.wang
 */
public class PluploadUtil {
	// logger
	private static Logger logger = LoggerFactory.getLogger(PluploadUtil.class);
	...
	/**
	 * @TODO 用于Plupload插件的文件上传,自动生成唯一的文件保存名(还未完成)
	 * 
	 ...
	 * @throws IOException
	 */
	public static InputStream uploadStream(Plupload plupload) throws IOException {
	 ...
	 * @throws IOException
	 */
	public static MultipartFile uploadMultiparFile(Plupload plupload) throws IOException {
		MultipartHttpServletRequest multipartRequest = (MultipartHttpServletRequest) plupload.getRequest();
		MultiValueMap<String, MultipartFile> map = multipartRequest.getMultiFileMap();
		if (map != null) {
			Iterator<String> iter = map.keySet().iterator();
	...
	public static void upload(Plupload plupload, File dir, String filename) throws IllegalStateException, IOException {
		...
		logger.debug(plupload.getMultipartFile() + "----------");
	...
	/**
	 * 
	 * @TODO 判断是否全部上传完成 碎片需合并后才返回真(还未完成)
	 *
	 * @param plupload plupload对象
	 * @return 判断结果
	 */
