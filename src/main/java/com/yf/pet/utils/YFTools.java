...
//                log(YFDataType.getString(entry.getKey()) + " : " + YFDataType.getString(entry.getValue()));
            }
        }
    } // End showMap
    ...
    public static int getIntValue(Integer value) {
        return YFTools.isEmpty(value) ? 0 : value.intValue();
    }
    ...
    public static double getDoubleValue(Double value) {
        return YFTools.isEmpty(value) ? 0 : value.doubleValue();
    }

    public static long getLongValue(Long value) {
        return YFTools.isEmpty(value) ? 0 : value.longValue();
    }

    public static String getStringValue(String value) {
        return YFTools.isEmpty(value) ? "" : value;
    }
    ...
    public static byte[] combine(byte[] source, byte[] append) {
        int oldLength = YFTools.isEmpty(source) ? 0 : source.length;
        int newLength = YFTools.isEmpty(append) ? 0 : append.length;
        byte[] resultData = new byte[oldLength + newLength];
        if (!YFTools.isEmpty(source)) {
            System.arraycopy(source, 0, resultData, 0, oldLength);
        }
        if (!YFTools.isEmpty(newLength)) {
    ...
    /**
     * url编码, 按RFC-1738, 空格变成"+", 主要用于底层编码, 千万别用在浏览器上面!
     *
     * @param s
     * @param enc
     * @return
    ...
    /**
     * url编码, 按RFC-2396, 空格变成"%20", 主要用于浏览器
     *
     * @param s
     * @param enc
     * @return
     */
    public static String encodeForBrowser(String s, String enc) {
        try {
            if (YFTools.isEmpty(s)) {
                return "";
            }
